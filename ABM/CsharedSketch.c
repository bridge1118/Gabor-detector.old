...
int numImage, numElementLimit; /* number of images and number of Gabors */   
double *numElementPointer, responseLimit; /* number of elements selected and lower limit on average response */    
...
void NonMaximumSuppression(int img, int mo, int mx, int my) 
{
   ...
   startx0 = (mx-2*halfFilterSize)-locationShiftLimit+1; 
   starty0 = (my-2*halfFilterSize)-locationShiftLimit+1; 
   endx0 =   (mx+2*halfFilterSize)+locationShiftLimit; 
   endy0 =   (my+2*halfFilterSize)+locationShiftLimit; 
   for (orient=0; orient<numOrient; orient++)   
     {
              ...
              if(Correlation[mo+orient1*numOrient]
                [px(x1-mx+2*halfFilterSize+1,y1-my+2*halfFilterSize+1,4*halfFilterSize+1,4*halfFilterSize+1)]==0.) 
                 {   /* if it is indeed inhibited */
                     maxResponse1 = LocalMaximumPooling(img, orient, x, y, trace); 
...
void InitializeMAX1map()
{
   ...
   pooledMax1map = float_matrix(numOrient, sizex*sizey);  
   MAX1map = float_matrix(numImage*numOrient, sizex*sizey);  
   trackMap = int_matrix(numImage*numOrient, sizex*sizey);   
   startx = (halfFilterSize+1)+1+locationShiftLimit; 
   endx = (sizex-halfFilterSize)-1-locationShiftLimit; 
   starty = (halfFilterSize+1)+1+locationShiftLimit; 
   endy = (sizey-halfFilterSize)-1-locationShiftLimit; 
      ...
      for (y=starty; y<=endy; y++)
       {
        ...
        for (orient=0; orient<numOrient; orient++)
           {    
             ...
             {
                i = orient*numImage+img; 
                MAX1map[i][here] = LocalMaximumPooling(img, orient, x, y, trace);    
                trackMap[i][here] = trace[0]; 
...
void PursueElement()
{
   ...
   { /* select the next Gabor */
     ...
     DrawElement(commonTemplate, besto, bestx, besty, sqrt(average));     
     here = px(bestx, besty, sizex, sizey); 
     for (img=0; img<numImage; img++)
        {    
          ...
          mx = bestx + xShift[besto][shift]; 
          my = besty + yShift[besto][shift];      
         if (maxResponse>0.)
            {  ///////////////////////////
  ...
  while ((t<numElementLimit)&&(average>responseLimit));  
  numElementPointer[0] = t;  
}
                 ...
                 int nrhs, const mxArray *prhs[])                
{
 ...
 numElementLimit = ROUND(mxGetScalar(prhs[c++])); 
 numElementPointer = mxGetPr(prhs[c++]); 
 responseLimit = mxGetScalar(prhs[c++]); 
 /* about input images */
 ...
 free_matrix(pooledMax1map, numOrient, sizex*sizey);  
 free_matrix(MAX1map, numImage*numOrient, sizex*sizey);  
 free_matrix(trackMap, numImage*numOrient, sizex*sizey);   
